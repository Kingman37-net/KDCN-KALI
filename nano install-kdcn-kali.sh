...
# ========== KDCN CUSTOM PROMPT FUNCTION ==========
function custom_prompt() {
    printf "${blue}[*] Setting KDCN custom prompt (kdcn-kali)...${reset}\n"
    # For kali user
    if [ -f ${CHROOT}/home/kali/.bashrc ]; then
        echo 'PS1="kdcn-(kali㉿kdcn-kali)-[\\w]\\$ "' >> ${CHROOT}/home/kali/.bashrc
    fi
    # For root user
    if [ -f ${CHROOT}/root/.bashrc ]; then
        echo 'PS1="kdcn-(root㉿kdcn-kali)-[\\w]\\# "' >> ${CHROOT}/root/.bashrc
    fi
    # Also set for future user skeleton
    if [ -d ${CHROOT}/etc/skel ]; then
        echo 'PS1="kdcn-(kali㉿kdcn-kali)-[\\w]\\$ "' >> ${CHROOT}/etc/skel/.bashrc
    fi
}
# ===============================================
...
create_kex_launcher
fix_uid
custom_prompt   # <-- THIS IS THE ADDED LINE
